            ...
            <Nav className="ml-auto">
              ...
              <Link to="/requests">
                {checkSupplier() ? null
                  : (
                    <div className="account-icon">
                      <FileCopy className="icon" fontSize="large" />
                      <NavDropdown title="Requests" className="nav-drop mr-3" id="drop-down">
                        <NavDropdown.Item className="dropdown-item" href="/requests">My Requests</NavDropdown.Item>
                        <NavDropdown.Divider />
                        {
                          (data && data.profile.role === 4)
                            ? (
                              <NavDropdown.Item className="dropdown-item" href="/manager/requests/applications">
                                <Link to="/manager/requests/applications">Request Applications</Link>
                              </NavDropdown.Item>
                            )
                            : ''
                        }
